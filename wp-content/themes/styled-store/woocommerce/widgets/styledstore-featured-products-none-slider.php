		...
		echo $args['before_widget'];
			echo '<div class="featured-product-noSlider">';
				...
				if ( ! empty( $instance['title'] ) ) {
					...
					echo do_shortcode('[featured_products per_page="'.esc_attr( $st_perpage ).'" columns="'. esc_attr( $st_column ).' orderby="'.esc_attr( $st_orderby ).'" order="'.esc_attr( $st_sort ).' category="'.esc_attr( $st_category ).'"]');
